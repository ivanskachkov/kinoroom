...
// Сводка о голосовой связи от браузера — одной строкой в журнал (journalctl -u kinoroom).
// Только тип соединения и цифры, без адресов: по ним видно, где ломается звук.
const LINK_STATES = new Set(['new', 'connecting', 'connected', 'disconnected', 'failed', 'closed']);
const num = (value, max = 100_000) => (Number.isFinite(value) ? Math.max(-1, Math.min(max, Math.round(value))) : null);
const clean = (value, max) => String(value ?? '').replace(/[^\p{L}\p{N} ./-]/gu, '').slice(0, max);
const printable = (value) => String(value ?? '').replace(/\p{C}/gu, '').slice(0, 30); // имя без переводов строк

export function formatVoiceReport(roomId, member, data, nameOf) {
  if (!data || typeof data !== 'object') return null;
  const parts = [`[голос] ${roomId} ${printable(member.name)} (${clean(data.ua, 40) || '?'})${data.live ? ' в эфире' : ''}`];
  const mic = data.mic;
  if (mic && typeof mic === 'object') {
    const open = num(mic.open, 100);
    parts[0] += ` · микрофон: ${clean(mic.meter, 10)}/${clean(mic.context, 12)}, порог ${Number(mic.threshold) ? Number(mic.threshold).toFixed(2) : '?'}${open === null ? '' : `, открыт ${open}%`}`;
  }
  for (const link of Array.isArray(data.links) ? data.links.slice(0, 12) : []) {
    if (!link || typeof link !== 'object' || !['in', 'out'].includes(link.dir)) continue;
    const who = printable(nameOf(typeof link.memberId === 'string' ? link.memberId : null));
    const facts = [LINK_STATES.has(link.state) ? link.state : '?'];
    if (link.net) facts.push(clean(link.net, 30));
    if (num(link.rtt) !== null) facts.push(`rtt ${num(link.rtt)} мс`);
    if (num(link.packets) !== null) facts.push(`пакетов ${num(link.packets)}`);
    if (num(link.loss, 100) !== null) facts.push(`потери ${num(link.loss, 100)}%`);
    if (num(link.conceal, 100) !== null) facts.push(`провалы ${num(link.conceal, 100)}%`);
    if (Number.isFinite(link.level)) facts.push(`уровень ${Math.max(0, Math.min(1, link.level)).toFixed(2)}`);
    if (link.dir === 'in' && typeof link.playing === 'boolean') facts.push(link.playing ? 'играет' : 'НЕ играет');
    parts.push(`${link.dir === 'out' ? '→' : '←'} ${who}: ${facts.join(', ')}`);
  }
  return parts.join(' | ');
}
...
export function attachRooms(io, { libraryEnabled, store = null }) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('voice:report', inRoom((data) => {
      if (!allow('voice-report', 10, 30_000)) return;
      const line = formatVoiceReport(room.id, member, data, (id) => room.members.get(id)?.name ?? '?');
