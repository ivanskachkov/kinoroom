...
// Громкость у каждого своя и не синхронизируется.
const volume = { level: storage.get('kr_volume', 1), muted: storage.get('kr_muted', false) };
...
function renderMembers() {
  ...
  list.replaceChildren(
    ...shown.map((m) =>
      el('span', { class: 'avatar', title: m.id === state.me?.id ? `${m.name} (вы)` : m.name, style: { background: m.color } }, m.name.slice(0, 1).toUpperCase()),
    ),
  );
